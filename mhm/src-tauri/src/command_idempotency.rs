...
pub use types::{
    ActorType, CommandLedgerErrorSummary, CommandLedgerResultSummary, CommandLedgerSummary,
    CommandStatus, IdempotentCommandResult, LedgerAggregateRef, LockKeyDeriver,
    ResolvedWriteCommandGuard, SanitizedLedgerIntent, WriteCommandContext, WriteCommandRequest,
    WriteCommandServiceFuture,
};
...
impl WriteCommandExecutor {
        ...
        F: for<'tx> FnOnce(&'tx mut Transaction<'_, Sqlite>) -> WriteCommandServiceFuture<'tx>,
    {
        ...
        let claim = claim_replay::claim_or_reclaim(&self.pool, ctx, &prepared).await?;
        ...
        F: for<'tx> FnOnce(&'tx mut Transaction<'_, Sqlite>) -> WriteCommandServiceFuture<'tx>,
    {
        ...
        if let Some(row) = claim_replay::fetch_existing_claim_tx(&mut tx, ctx).await? {
            let outcome =
                claim_replay::resolve_existing_claim_row(ctx, &prepared.request_hash, row);
