...
Before pushing a release tag, run the [release checklist](release-checklist.md). This signing guide covers updater keys, platform assets, and publishing mechanics; the checklist covers baseline validation, smoke validation, and normal PMS release readiness.
...
### Optional (required only for encrypted TAURI key)
...
Store the private key output as `TAURI_SIGNING_PRIVATE_KEY` and copy the matching printed public key from the same run into `TAURI_UPDATER_PUBLIC_KEY`.

Rerun this command only when you are intentionally rotating keys, and then update both values together.
