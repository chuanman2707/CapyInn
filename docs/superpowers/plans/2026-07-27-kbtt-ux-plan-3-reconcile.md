...
Spec §4.3: khách của lô `failed` không tự quay lại danh sách (tránh xuất trùng), nhưng nếu chính DỮ LIỆU trong file sai thì "upload lại đúng file đó" là ngõ cụt. Thẻ lô `failed` vì vậy có nút "Đưa khách về danh sách để sửa & xuất lại".
...
**Interfaces:**
- Produces: repo `pub async fn reopen_failed_batch(pool, batch_id: &str) -> Result<(), String>`; command `kbtt_reopen_batch(batch_id)`. Chỉ chạy với lô `status = 'failed'`; xóa entry của lô (khách quay lại `pending_link_ids` vì hết entry); dòng lô GIỮ NGUYÊN làm lịch sử.
...
- [ ] **Step 2: Chạy đỏ** — `cargo test --lib reopening_a_failed 2>&1 | tee /tmp/kbtt-p3t1.log; echo "EXIT=$?"`

- [ ] **Step 3: Viết repo fn (cạnh `set_batch_failed`)**
...
`lib.rs`: thêm vào `generate_handler!`.

- [ ] **Step 5: PASS + commit**

```bash
git add src/declaration/repo.rs src/commands/declaration.rs src/lib.rs
git commit -m "feat(kbtt): reopen a failed batch so its guests can be fixed and re-exported"
```
